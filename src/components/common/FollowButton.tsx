...
export function FollowButton({
  ...
  const baseStyle =
    className || "px-4 py-1.5 rounded-full text-xs font-bold transition hover:opacity-80";
  ...
  const getVariantStyle = () => {
    ...
    if (variant === "secondary") {
      return isFollowing
        ? "bg-[#151515] border border-white/10 text-white/70"
        : "bg-gradient-to-r from-[#FF9D2E] to-[#FF6B35] text-black";
    }
    // primary
    return isFollowing
      ? "bg-[#151515] border border-white/10 text-white/70"
