...
import { Mic, Users, Music, Radio, Plus, Volume2, X, Star } from "lucide-react";
import { ImageWithFallback } from "./ImageWithFallback";
import { KARAOKE_ROOMS, USERS } from "./data";
      ...
      <div className="flex items-center justify-between px-5 pt-6 pb-4">
        <div>
          ...
          <p className="text-white/40 text-xs mt-0.5">
            {room.genre} · {room.listeners} listening
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        ...
        <div className="bg-white/5 rounded-2xl px-5 py-4 mb-6 text-center border border-white/10 w-full max-w-xs">
          <div className="flex items-center justify-center gap-2 mb-1">
            <Music size={14} className="text-[#FF9D2E]" />
        ...
        <div className="w-full">
          <p className="text-white/30 text-[10px] uppercase tracking-widest text-center mb-3">
            Audience
          </p>
    ...
    <div className="min-h-full pb-28">
      ...
      <div className="px-4 mb-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[11px] uppercase tracking-widest text-white/40">Live Now</span>
          <div className="w-2 h-2 rounded-full bg-[#FF6B6B] animate-pulse" />
        </div>
      </div>

      <div className="px-4 space-y-3">
        {KARAOKE_ROOMS.map((room, i) => {
          const host = getUserByUsername(room.host);
          return (
            <motion.button
              key={room.id}
              ...
              className="w-full bg-[#151515] rounded-[20px] p-4 border border-white/5 text-left hover:border-white/10 transition"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/60 text-xs mt-1 truncate">♪ {room.currentSong}</p>
      ...
      <AnimatePresence>
        {activeRoom && <ActiveRoom room={activeRoom} onClose={() => setActiveRoom(null)} />}
      </AnimatePresence>
    </div>
  );
}
